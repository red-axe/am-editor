...
function applyDelta(
	...
	for (const change of changes) {
		if ('attributes' in change && 'retain' in change) {
			...
			for (
				...
				if (
					Text.isText(child) &&
					(pathOffset === startPathOffset ||
						pathOffset === endPathOffset)
				) {
					const start =
						pathOffset === startPathOffset ? startTextOffset : 0;
					const end =
						pathOffset === endPathOffset
							? endTextOffset
							: child.text.length;
		...
		if ('insert' in change) {
			const [pathOffset, textOffset] = yOffsetToEditorOffsets(
				node,
				yOffset,
				{
					insert: true,
...
export function translateYTextEvent(
	...
	const ops: Operation[] = [];
	const editorPath = getEditorPath(sharedRoot, editor.model.root, target);
	const targetElement = Node.get(editor.model.root, editorPath);
