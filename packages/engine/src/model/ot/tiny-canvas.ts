...
class TinyCanvas implements TinyCanvasInterface {
  ...
  constructor(options: Options) {
    if (!options.container) throw new Error('please set a dom cantainer!');
    ...
    if (index !== canvasCount) {
      ...
      for (let i = 0; i < index; i++) {
        ...
        if (i === index - 1) {
          ...
          canvas.setAttribute('height', (limitHeight || 0).toString());
        }
      ...
      if (canvas) {
        canvas.setAttribute('height', (height % (limitHeight || 0)).toString());
      }
    }
  }
