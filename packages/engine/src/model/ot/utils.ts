...
import { FOCUS, ANCHOR, CURSOR } from '../../constants/bookmark';
...
export const isTransientElement = (node: Node | NodeInterface) => {
  ...
  if (node.isElement()) {
    //范围标记
    if ([CURSOR, ANCHOR, FOCUS].includes(node.attr(DATA_ELEMENT))) return true;
    //data-transient属性
    if (node.attr('data-transient')) return true;
    //在卡片里面
    if (!node.isCard() && node.closest(CARD_SELECTOR).length > 0) return true;
    //当前是卡片，父级也是卡片
...
export const affectPath = (path: Path, otherPath: Path) => {
  ...
  while (path[index]) {
    ...
    if (path[index] === otherPath[index]) index++;
      ...
      if (path[index] < otherPath[index]) return path[index + 1] === undefined;
    }
  }
  return true;
};
