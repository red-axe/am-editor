...
class Creator extends EventEmitter2 {
  ...
  isTransientMutation(record: MutationRecord) {
    ...
    if (type === 'childList') {
      ...
      if (removedNodes[0] && isTransientElement(removedNodes[0])) return true;
    ...
    for (let i = 0; records[i]; ) {
      ...
      if (type === 'childList') {
        if (node.equal(target)) {
          if (removedNodes[0]) {
            const removeIndex = this.getRemoveNodeIndex(record, records);
          ...
          if (addedNodes[0]) {
            Array.from(addedNodes).forEach(addedNode => {
              ...
              if (addedNode.parentNode === node.get()) {
                let p: Path = [];
                p = p.concat([...path], [domAddedNode.getIndex() + 2]);
                pushAndRepair(ops, {
                  li: data,
                  p,
                  newPath: p.slice(),
        ...
        if (node.equal(target)) {
          if (!isDataString) {
            if (typeof getPathValue(this.doc?.data, path) === 'string') {
              attrOps.push({
                path,
        ...
        if (node.equal(target)) {
          ...
          if (oldValue !== attrValue) {
            const p: Path = [];
            const newOp: any = {};
            newOp.p = p.concat([...path], [1, escapeDots(attributeName || '')]);
  ...
  getRemoveNodeIndex(record: MutationRecord, records: MutationRecord[]) {
  ...
  getRemoveNodeIndexFromMutation(
    ...
    const record = records.find(
      record => record.target === target && record.removedNodes[0] === node,
    );
    if (record) {
