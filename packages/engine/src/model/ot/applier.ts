...
class Applier implements ApplierInterface {
  ...
  insertNode(path: Path, value: string | Op[] | Op[][]) {
    const { engine } = this;
    const [begine, beginOffset, end] = elementAtPath(engine.container[0], path);
  ...
  insertInText(path: Path, offset: number, text: string) {
    ...
    switch (endOffset) {
      ...
      default:
        ...
        const nodeValue = begine && begine.nodeValue ? begine.nodeValue : '';
        const value =
          nodeValue.substring(0, offset) + text + nodeValue.substring(offset);
        if (begine && begine.parentNode === end) begine.nodeValue = value;
        else {
  ...
  setRangeByRemotePath(path: RemotePath) {
    try {
      ...
      const range = selection ? Range.from(selection)?.cloneRange() : undefined;
      if (!range) return;
