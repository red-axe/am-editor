    ...
    if (Array.isArray(ops[i])) {
      ...
      if (elementName) {
        ...
        for (let attr in ops[i]) {
          element!.setAttribute(unescapeDots(attr), unescape(ops[i][attr]));
        }
      }
...
export const fromDOM = (node: NodeInterface) => {
  ...
  if (!isTransientElement(node)) {
    const { nodeName, nodeType, attributes, nodeValue } = node.get<Element>()!;
      ...
      if ('ld' in opItem) {
        if (itemOffset <= opOffset) {
          ...
          itemOldPath[itemOldPath.length - 1] =
            parseInt(itemOldPath[itemOldPath.length - 1].toString()) + 1;
