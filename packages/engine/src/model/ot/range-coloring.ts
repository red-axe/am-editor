...
class RangeColoring implements RangeColoringInterface {
  ...
  drawBackground(
    ...
    if (child && child.length > 0) {
      child.attr('data-color', color.toString());
  ...
  getCursorRect(
    ...
    const parentRect = this.root.get<Element>()?.getBoundingClientRect() || {
      top: 0,
      left: 0,
    };
  ...
  drawCardMask(node: NodeInterface, cursor: NodeInterface, member: Member) {
    ...
    mask = $(`<div class="${USER_MASK_CLASS}" data-uuid="${member.uuid}" />`);
    mask[0]['__targetNode'] = node[0];
    mask.css({
      left: nodeRect.left - parentRect.left + 'px',
      top: nodeRect.top - parentRect.top + 'px',
  ...
  drawRange(range: RangeInterface, member: Member) {
    ...
    card.each(cardComponent => {
      if (!cardInfo || !cardComponent.root.equal(cardInfo.root)) {
        ...
        this.root.children(`.${USER_MASK_CLASS}[data-uuid="${uuid}"]`).remove();
      }
    });
    if (cardInfo) {
      const root =
      ...
      if (singleCard) {
        const root =
          this.setCardSelectedByOther(singleCard, member) || singleCard.root;
  ...
  updateCursorPosition() {
    this.root.children(`.${USER_CURSOR_CLASS}`).each(child => {
      ...
      if (target.startContainer || 0 !== $(target).closest('body').length) {
  ...
  render(data: Array<Attribute>, members: Array<Member>, idDraw: boolean) {
    ...
    data.forEach(item => {
      ...
      if (member && (idDraw || active)) {
        if (path.length !== 0) {
          const range = fromPath(engine.container, path);
    ...
    this.root.children('[data-uuid]').each(child => {
      ...
      const uuid = domChild.attr('data-uuid');
      const member = members.find(m => m.uuid === uuid);
      if (!member || info[uuid]) {
    ...
    engine.card.each(component => {
      const member = members.find(m => m.uuid === component.selectedByOther);
