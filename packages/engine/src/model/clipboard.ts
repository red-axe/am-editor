...
export default class Clipboard implements ClipboardInterface {
  ...
  getData(event: DragEvent | ClipboardEvent) {
    ...
    if (html === '' && text && /^.+\.\w+$/.test(text) && files.length > 0) {
      ...
      (html || text) &&
      files.length > 0 &&
      !html?.startsWith('<img') && (html || !/^https(s)?:/.test(text || ''))
    ) {
      files = [];
    }
    return {
  ...
  write(
    ...
    if (card.length > 0) {
      const cardCenter = root.closest('[data-card-element="center"]', node => {
        return $(node).isRoot() ? undefined : node.parentNode || undefined;
      });
      if (cardCenter.length === 0) {
        range.select(card);
    ...
    if (card.length <= 0 && (hasChildEngine || hasParentEngine)) {
      if (hasParentEngine && !root.isRoot()) {
        ...
        if (
          ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].indexOf(block.name || '') > -1
        ) {
      ...
      if (range.collapsed) {
        ...
        const parser = new Parser(contents, this.engine, this.contentView);
        let { html, text } = parser.toHTML(inner, outter);
        if (callback) {
          callback({ html, text });
        }
        if (html) html = '<meta name="source" content="aomao" />' + html;
  ...
  copy(data: Node | string, trigger: boolean = false) {
    ...
    if (trigger) {
      block.allChildren().forEach(child => {
        Object.keys(
          (this.engine || this.contentView)!.plugin.components,
        ).forEach(name => {
          const plugin = (this.engine || this.contentView)!.plugin.components[
            name
          ];
