...
class HistoryModel implements HistoryInterface {
  ...
  undo() {
    ...
    if (undoOp) {
      ...
      try {
        ...
        if (lastOp && lastOp.uid === undoOp.uid && lastOp.status !== 'undo') {
          this.totalOps.pop();
        } else {
  ...
  redo() {
    ...
    if (redoOp) {
      ...
      try {
        ...
        const lastOp = this.totalOps[this.totalOps.length - 1];
        if (lastOp && lastOp.uid === redoOp.uid && lastOp.status === 'undo') {
          this.totalOps.pop();
        } else {
  ...
  getUndoOp(): Operation | undefined {
    ...
    if (this.actionOps[prevIndex]) {
      ...
      let opIndex = findLastIndex(this.totalOps, op => op.uid == prevOp.uid);
    ...
    if (this.actionOps[currentIndex]) {
      ...
      try {
        ...
        return {
          ...
          startRangePath: transformPath(currentOp.startRangePath, ops),
        };
      } catch (error) {
        console.log(error);
      }
