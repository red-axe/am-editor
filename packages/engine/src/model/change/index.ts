...
class ChangeModel implements ChangeInterface {
  ...
  setValue(value: string, onParse?: (node: Node) => void) {
    ...
    if (value === '') {
      ...
      container.htmlKeepID(
        parser.toValue(schema.getValue(), conversion.getValue(), false, true),
      );
  ...
  private repairInput(range: RangeInterface) {
    ...
    if (card && card.type === CardType.INLINE) {
      if (card.isLeftCursor(commonAncestorNode)) {
        ...
        const cardRight = commonAncestorNode.closest(CARD_RIGHT_SELECTOR);
  ...
  private initNativeEvents() {
    ...
    this.event.onSelect(() => {
      ...
      this.select(range);
      this.activateCard(range.commonAncestorNode, ActiveTrigger.CUSTOM_SELECT);
      this.onSelect();
    });
    ...
    this.event.onPaste(data => {
      ...
      if (files.length === 0) {
        ...
        if (isPasteText) {
          let value = '';
          if (text) value = text;
          else if (html) value = new Parser(html, this.engine).toText();
          source = new TextParser(value).toHTML();
        } else {
          // 富文本粘贴
          if (
            html &&
            html.indexOf('<meta name="source" content="aomao" />') > -1
          ) {
            source = html;
    ...
    const insertCardAble = (range?: RangeInterface) => {
      ...
      return !range || this.engine.card.closest(range.commonAncestorContainer);
    };
  ...
  activateCard(
    ...
    if (!container.get() || this.engine.container.equal(container)) {
      ...
      if (card) {
        ...
        if (!isCurrentActiveCard && trigger === ActiveTrigger.MOUSE_DOWN) {
