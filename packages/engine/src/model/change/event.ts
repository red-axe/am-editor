...
class ChangeEvent implements ChangeEventInterface {
  ...
  onSelect(callback: EventListener) {
    ...
    this.onContainer('keyup', e => {
      ...
      if (
        ...
        const range = Range.from();
        if (this.keydownRange && range && range.equal(this.keydownRange))
          return;
        if (!this.isComposing) {
          callback(e);
        }
  ...
  onPaste(callback: (data: ClipboardData & { isPasteText: boolean }) => void) {
  ...
  destroy() {
    this.events.forEach(item => {
      if (item.type === 'window') {
        window.removeEventListener(item.eventType, item.listener, item.rewrite);
      }
      ...
      if (item.type === 'container') {
        ...
        if (unbindContainer) unbindContainer(item.eventType, item.listener);
      }
    });
  }
}
