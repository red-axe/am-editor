...
import { ANCHOR_SELECTOR, FOCUS_SELECTOR } from '../../../constants/bookmark';
      ...
      const handleAt = (node: Node | null, align: boolean) => {
        ...
        while (node) {
          ...
          if (node.textContent === atText) {
            ...
            if (align) {
              while (atText && node.textContent?.endsWith(atText)) {
              ...
              while (atText && node.textContent?.startsWith(atText)) {
...
const splitMarkAtExpandedRange = (
  ...
  if (
    !(
      (cardStart.length > 0 && 'inline' === cardStart.attr(CARD_TYPE_KEY)) ||
