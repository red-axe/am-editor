...
import { canRemoveMark, unwrapNode, getDocument, getWindow } from '../../utils';
...
export default (
	...
	if (mark !== undefined && (typeof mark === 'string' || isNode(mark))) {
		mark = $(mark, doc);
	}
	...
	markNodes.forEach(node => {
		...
		if (canRemoveMark(node, markNode)) {
			unwrapNode(node);
		} else if (markNode) {
			const styleMap = markNode.css();
			Object.keys(styleMap).forEach(key => {
				node.css(key, '');
			});
			if (
				markNode.get<Element>()?.className.indexOf('data-fontsize') !==
				-1
			) {
				nodeChild = node;
				className =
