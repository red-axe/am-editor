...
import { unwrapNode, setNode, wrapNode, getWindow } from './node';
...
import { CardModelInterface, CardType } from '..';
...
export const repairListStylePosition = (
  blocks: Array<NodeInterface>,
  align: 'left' | string,
) => {
...
const addBrToCustomzieList = (root: NodeInterface) => {
  root.find('ul>li').each(child => {
    ...
    while (node) {
      const domNode = $(node);
      //自定义列表项，第一个是card
      if (domChild.first()?.equal(domNode) && domChild.first()?.isCard()) {
        domChild.append('<br />');
        return;
      }
...
export const repairCustomzieList = (range: RangeInterface) => {
  if (range.collapsed) {
    ...
    if (startBlock1[0] !== startBlock2[0]) addBrToCustomzieList(startBlock2);
  }
};
